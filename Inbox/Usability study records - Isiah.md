@ -0,0 +1,65 @@
*2025-12-17*

*Status:* New

*Tags:* #UX #knowledge #research #usability_study

*Project:* [[Google UX design course|GUX]] Grocery Locator 

<hr>

## Questions

- Do you live close to any big grocery stores (like Auchan, Carrefour, Eurospar)?  
	 -- No, but I do live close to Lidl, Biedronka
- - Do you go there often and how often?
	 -- One time a week, maybe more
- Do you usually go alone or with someone?
	 -- Alone

## Prompts for tasks performed

- Prompt 1: Imagine you're at a store and you're struggling to find a product. Let's say you need vegan milk. Now open up the Grocery Locator app on your phone and try to find and locate it.
- Prompt 2: Now, let's take a different situation: you're making a grocery list before going shopping. Try to create one and add that 2 bottles of Vegan milk there. 
- Prompt 3: Now let's say before you buy that milk, you want to check if there are any sales for one of the brands of milk.
- Prompt 4: Now let's go back to that list you were making earlier. Let's imagine these recommended products are also worth buying, so let's add them, and try use navigator to shop. How do you think you can do that?

## KPI's in tasks

### 1. Task success rate

Task 1. Partial success

Task 2. Success

Task 3. Partial success

Task 4. Success

### 2. Time on task

Task 1. 1:00

Task 2. 0:35

Task 3. 1:35

Task 4. 0:30

### 3. User errors 

Task 1. Didn't find a way to check for vegan (used imagination, *as if* the other one was vegan)

Task 2. Everything went well besides user trying to add regular milk (since they didn't find the vegan one)

Task 3. Checked Promotions screen and Home screen, but didn't use search at all: "That doesn't track for me. Sorting is something I do for prices, or popularity, not promotions."

Task 4. Everything went smoothly. 

## Suggestions

- Sort sales into categories at the top, like Biedronka
- Add the ability to search for promotions.
- Check out Pepper

## Also check out / References